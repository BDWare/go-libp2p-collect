...
func mustNewTopicBasedOverlay(h host.Host) *TopicBasedOverlay {
	out, err := NewTopicBasedOverlay(h)
	if err != nil {
		panic(err)
	}
	return out
...
func mustJoin(t *TopicBasedOverlay, topic string) {
	err := t.Join(topic)
	if err != nil {
		panic(err)
	}
...
func mustLeave(t *TopicBasedOverlay, topic string) {
	err := t.Leave(topic)
	if err != nil {
		panic(err)
	}
...
func TestBasicJoinAndLeave(t *testing.T) {
	...
	tbo1 := mustNewTopicBasedOverlay(h1)
	tbo2 := mustNewTopicBasedOverlay(h2)
	topic := "test"
